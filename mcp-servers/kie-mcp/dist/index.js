...
function httpRequest(method, urlStr, headers, body, redirectCount = 0, asBuffer = false) {
    return new Promise((resolve, reject) => {
        ...
        if (body !== undefined && reqHeaders["Content-Length"] === undefined) {
            reqHeaders["Content-Length"] = String(Buffer.isBuffer(body) ? body.length : Buffer.byteLength(body));
        }
        const req = lib.request({
            ...
            if ([301, 302, 303, 307, 308].includes(status) &&
                ...
                resolve(httpRequest(newMethod, nextUrl, headers, newBody, redirectCount + 1, asBuffer));
                return;
            }
            const chunks = [];
...
async function kieUploadFile(args) {
    ...
    try {
        ...
        throw new Error(`Could not read file at ${absPath}: ${e instanceof Error ? e.message : String(e)}`);
    }
    const fileName = path.basename(absPath);
    const contentType = guessContentType(absPath);
    const boundary = `----dainami${Date.now()}${Math.random().toString(16).slice(2)}`;
    const fields = uploadPath ? { uploadPath } : {};
    const body = buildMultipartBody(fields, "file", fileName, fileBuffer, contentType, boundary);
    const res = await httpRequest("POST", KIE_UPLOAD_URL, {
        Authorization: `Bearer ${KIE_API_KEY}`,
    ...
    const url = (parsed && parsed.data && (parsed.data.downloadUrl || parsed.data.url)) || null;
    return {
...
async function kieDownload(args) {
    ...
    const res = await httpRequest("GET", url, { "Accept-Encoding": "identity" }, undefined, 0, true);
...
function sanitizeCacheSegment(segment) {
    // strip any leading/trailing slashes, collapse dangerous chars
    return segment
        .replace(/^[\/.]+|[\/.]+$/g, "")
        .replace(/[^a-zA-Z0-9_\-./]/g, "_");
}
function docsCachePathFor(url) {
...
async function readCacheIfFresh(cachePath, ttlMs) {
    try {
...
async function kieFetchModelDocs(args) {
...
`.trim();
const server = new Server({ name: "dainami-kie-mcp", version: "0.3.0" }, {
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "kie_post",
            description: "POST to any KIE.ai endpoint. Used to submit generation tasks. Returns { status, ok, body }. For payload shapes, read kie://models/<name>.",
            inputSchema: {
                type: "object",
                properties: {
                    path: {
        ...
        {
            name: "kie_get",
            description: "GET from any KIE.ai endpoint. Typically used for polling task status (e.g. '/api/v1/jobs/recordInfo?taskId=...'). Returns { status, ok, body }.",
            inputSchema: {
                type: "object",
                properties: {
                    path: {
        ...
        },
        {
            ...
            description: "Upload a local file to KIE's storage and get back a hosted URL (~3 day TTL). Use whenever a model needs an @Image/@Video reference and you only have a local path. Returns { ok, status, url, bytes }.",
            inputSchema: {
                type: "object",
                properties: {
                    localPath: {
                        type: "string",
                        description: "Path to the local file. Absolute, or relative to the MCP process's working directory.",
                    },
                    uploadPath: {
                        type: "string",
        ...
        {
            name: "kie_download",
            description: "Download a URL (typically a KIE result URL) to a local file. Creates parent directories. Returns { ok, status, destPath, bytes }.",
            inputSchema: {
                type: "object",
                properties: {
                    ...
                    destPath: {
                        type: "string",
                        description: "Local file path to write. Absolute, or relative to the MCP process's working directory.",
                    },
                },
        ...
        },
        {
            ...
            description: "Fetch a KIE.ai model's live documentation page (from docs.kie.ai/market/...). Result is cached locally for ~3 days so re-use across chats is free. Use this whenever you're about to call a model whose payload shape you don't already know in the current session. Returns { ok, url, cached, content }.",
            inputSchema: {
                type: "object",
                properties: {
                    path: {
                        type: "string",
                        description: "Docs path under /market — e.g. 'google/nanobanana2', 'bytedance/seedance-2', 'wan/2-7-image-to-video'. Look up the right path in kie://models/_index.",
                    },
                    url: {
                        type: "string",
                        description: "Full docs URL. Alternative to path. Useful for non-/market namespaces like /veo3-api/quickstart, /suno-api/generate-music.",
                    },
                    force: {
                        type: "boolean",
                        description: "Force a fresh fetch, bypassing the 3-day local cache. Use this if a previous call to the model failed with a parameter error and you suspect the cached spec is stale. Default false.",
                    },
                },
            },
        },
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    try {
        if (name === "kie_post") {
            const result = await kieFetch("POST", args.path, args.body);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        }
        if (name === "kie_get") {
            const result = await kieFetch("GET", args.path);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        }
        if (name === "kie_upload_file") {
            const result = await kieUploadFile(args);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        }
        if (name === "kie_download") {
            const result = await kieDownload(args);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        }
        if (name === "kie_fetch_model_docs") {
            const result = await kieFetchModelDocs(args);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
        }
        return {
...
console.error(`[dainami-kie-mcp] running on stdio (v0.3.0 — live-docs discovery via kie_fetch_model_docs)`);
